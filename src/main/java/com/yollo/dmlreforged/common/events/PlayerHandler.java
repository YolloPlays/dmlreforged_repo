    ...
    public static void playerRightClickedBlock(PlayerInteractEvent.RightClickBlock event) {
    	if(!event.getPlayer().isCrouching()) {
    		...
    		if(event.getItemStack().getItem() instanceof ItemGlitchHeart && rand.nextInt(0, 10) <= 3) {
        		...
        		BlockPos blockPos = event.getPos();
        		if(event.getPlayer().getLevel().getBlockState(event.getPos()).getBlock() == Blocks.COAL_BLOCK && rand.nextInt(0, 10) <= 3) {
        		...
        		event.getPlayer().getLevel().addParticle(ParticleTypes.FLAME, (double) blockPos.getX()+ rand.nextDouble(0,1),(double) blockPos.getY()+1,(double) blockPos.getZ()+rand.nextDouble(0,1), 0d, 0.03d, 0d);
        		event.setCanceled(true);
    		}
    	}
	}
